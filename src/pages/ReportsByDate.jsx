    ...
    const fetchData = async () => {
      try {
        ...
        const grouped = data.reduce((acc, transaction) => {
          ...
          acc[dateKey].dailyTotal += (transaction.totalAmount || 0);
          return acc;
        }, {});
                      ...
                      Object.keys(groupedTransactions).map((date) => (
                        <React.Fragment key={date}>
                          ...
                          <tr className="bg-gray-50">
                            <td colSpan="2" className="p-4 font-bold text-gray-700 border-l-4 border-blue-500">
                              {date}
                            </td>
                            <td className="p-4 text-right font-bold text-blue-600">
                              Total: Rp {groupedTransactions[date].dailyTotal.toLocaleString()}
                            </td>
                          </tr>
                          ...
                          {groupedTransactions[date].items.map((t) => (
                            <tr key={t.id} className="hover:bg-gray-50/50 transition">
                              <td className="p-4 pl-8 text-gray-500">
                                {t.date ? (t.date?.toDate ? t.date.toDate() : new Date(t.date)).toLocaleTimeString('id-ID', {
